...
```{r}
ggplot(phone_df, aes(x=price_range, y=clock_speed))+ geom_boxplot() + labs(title = "Battery power distribution by Price range") 
...
```
From the metric analysis we note that we have 93.87% accuracy with the confusion matrix showing a few misclassified cases.
...
confusionMatrix(cm)
```
Using K at 44 we find a accuracy of 91.6%. We then try and find the optimum number of k
...
```
Doing tuning to find the optimum number of neighbour to be 13.


#### using optimum k at 13
```{r}
# Fitting KNN Model 
# to training dataset
...
confusionMatrix(cm2)
```
From the tuned number of neighbours we find a slightly increased accuracy of 92.65.
...
```
By tuning the number of K neighbors we find that:
